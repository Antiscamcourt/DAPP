...
contract AntiScamCourtDapp{
    ...
    */
    function markScam(uint _id)onlyCurator(msg.sender)  public {// only curator
        ...
        require(dappsById[_id].dappStatus==status.inconclusive,"You can only markscam an inconclusive dapp.");
        dappsById[_id].dappStatus=status.scammed;
        dappsById[_id].isActive=false;
        dappsById[_id].remarks="Marked Scam by the Curator.";
    ...
    */
    function  swapAndstakeSTT(uint tokens)public {
        ...
        uint sttToStake = (tokens-commission)*96/100;
        _STTstakeBalance[staker] +=(tokens-commission)*96/100;
    ...
    */
     function  redeem(uint tokens)public {
        ...
        stakersReward(staker,commission);// removed general reward
    }
   ...
   function addCurator(address curator) onlyOwner(msg.sender)public{
    ...
    spendingSTT[loosers[i]] -=returnedvoting;
    _STTstakeBalance[loosers[i]]-= (votingFees-returnedvoting);
...
for(uint i=0;i<winners.length;i++){
    ...
    _STTstakeBalance[winners[i]]-= (votingFees-returnedvoting);// no net fees is being charged from the staked STP
    _STTstakeBalance[winners[i]]+= rewardperJuror;// rewarding the winning jurors, by crediting the STP
...
function withdraw(uint amount) onlyOwner(msg.sender)public{
    uint balance=contractAST.balanceOf(address(this));
    require(amount<=balance,"dont have enoughSTT to redeem");
